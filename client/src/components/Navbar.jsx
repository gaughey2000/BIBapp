  ...
  return (
    <header
      className="sticky top-0 z-40
                 ...
                 border-b border-[color:var(--rose)]/30
                 shadow-sm"
    >
      ...
      {open && (
        <div
          className="md:hidden backdrop-blur-md bg-white/55 border-t border-[color:var(--rose)]/30"
        >
          <nav className="container-narrow py-2 grid gap-1">
            <NavLink to="/book" onClick={() => setOpen(false)} className={({isActive}) => `${linkBase} ${isActive ? active : inactive}`}>Book</NavLink>
            <NavLink to="/services" onClick={() => setOpen(false)} className={({isActive}) => `${linkBase} ${isActive ? active : inactive}`}>Services</NavLink>
